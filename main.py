...
class SearchCourseHandler(webapp2.RequestHandler):
    def get(self):
        ...
        conn2 = rdbms.connect(instance=_INSTANCE_NAME, database='Prinya_Project')
        cursor2 = conn2.cursor()
        sql2="SELECT title from faculty"
...
class SearchHandler(webapp2.RequestHandler):
    def get(self):
        ...
        data_faculty=self.request.get('faculty');
        data_faculty=str(data_faculty)
        ...
        if data_faculty =="":
            ...
            check_fac=1
            data_faculty_full="faculty='"+data_faculty+"'";
...
class EnrollHandler(webapp2.RequestHandler):
    def get(self):
    	...
    	conn4 = rdbms.connect(instance=_INSTANCE_NAME, database='Prinya_Project')
        cursor4 = conn4.cursor()
        sql4="SELECT maxcredit_per_semester from faculty f join student s on f.faculty_id=s.faculty_id where student_id='%d'"%(student_id)
        cursor4.execute(sql4)
        credit_total=0
